...
- **Reactive store metadata signals** — stores now expose reactive `history`, `messages`, `currentIndex`, and `keys` signals on the public store contract, making devtools and other UI observers subscribe without polling snapshot getters.

### Fixed

- **Public store typings** — the public reactive store contract now preserves typings correctly for signal-backed stores after the `1.0.0` release, including replay/history access through the shared interface.

### Changed

- **Store internals** — constructor field typing and history-driver wiring were refactored to support the new reactive metadata signals without changing the release line's public package layout.
